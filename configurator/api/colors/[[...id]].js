...
// library's folder tree (`?folders=1`, kind='color') rather than adding
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!id) {
      if (isFolders) {
        if (req.method === 'GET') {
          ...
          const rows = await sql`select * from folders where owner_id = ${targetOwnerId} and kind = 'color' order by name asc`;
        ...
        if (req.method === 'POST') {
          ...
          const [row] = await sql`
            ...
            values (${userId}, 'color', ${parentId || null}, ${name})
            returning *
          `;
    ...
    if (isFolders) {
      const [existing] = await sql`select owner_id from folders where id = ${id} and kind = 'color'`;
