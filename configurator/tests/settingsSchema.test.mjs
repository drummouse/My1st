...
test('settings.id is the primary key, not the vestigial singleton column', () => {
  for (const source of [runtimeSchema, referenceSchema]) {
    ...
    // Two lanes independently fixed this bug; the integration merge kept the
    // more defensive implementation (dynamic constraint discovery — see
    // superadminSchema.test.mjs for its invariants). These fragments assert
    // that kept implementation's DDL, verifying the same end-state: the
    // primary key moves to `id` and `singleton` is demoted, not dropped.
