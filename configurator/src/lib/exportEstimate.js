...
export const formatPct = (rate) => (rate * 100).toFixed(2).replace(/\.?0+$/, '');
...
export function buildEstimateText({
  ...
  estimate.appliedDiscounts
    ...
    .forEach((d) => lines.push(`${d.name} (${Math.round(d.pct * 100)}%): -${money(d.amount)}`));
  ...
  lines.push(`${estimate.taxLabel} (${formatPct(estimate.taxRate)}%): ${money(estimate.taxAmount)}`);
  lines.push(`TOTAL ESTIMATE: ${money(estimate.total)}`);
  if (estimate.appliedDiscounts.length) {
    lines.push('');
    estimate.appliedDiscounts.forEach((d) => lines.push(d.summary));
  }
  if (attachments?.length) {
    lines.push('');
    lines.push('ATTACHMENTS');
    lines.push('-'.repeat(50));
    attachments.forEach((a) => lines.push(`${a.kind === 'photo' ? 'Photo' : 'File'}: ${a.file_name} — ${a.url}`));
  }
