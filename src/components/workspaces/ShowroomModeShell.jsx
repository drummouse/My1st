...
export function PresentationCatalogEditor({ controls }) {
  ...
  const isServiceCategory = controls.selectedCategory === 'services';
  const productLabel = isServiceCategory ? 'Service' : 'Product';
  ...
  return (
    <section aria-label="Presentation catalog editor" className="showroom-presentation-editor control-block">
      <div className="showroom-section-heading">
        ...
        <h2>{isServiceCategory ? 'Services' : 'Products and profiles'}</h2>
      </div>
      ...
      <label>
        ...
        >
          {!selectedProductId && <option value="">Choose a {productLabel.toLowerCase()}</option>}
...
export const colorLineForMaterial = (material) => material?.series?.trim() || 'Standard';

export function ShowroomColorLineShelf({ materials = [] }) {
  const lines = [...new Set(materials.map(colorLineForMaterial))];
  const selectedMaterial = materials.find((material) => material.selected === true);
  const preferredLine = colorLineForMaterial(selectedMaterial || materials[0]);
  const materialFingerprint = materials.map((material) => (
    `${material.id}:${colorLineForMaterial(material)}:${material.selected === true}`
  )).join('|');
  const [activeLine, setActiveLine] = useState(preferredLine);
  const previousFingerprintRef = useRef(materialFingerprint);

  useEffect(() => {
    if (previousFingerprintRef.current === materialFingerprint) return;
    previousFingerprintRef.current = materialFingerprint;
    if (!lines.includes(activeLine) || selectedMaterial) setActiveLine(preferredLine);
  }, [activeLine, materialFingerprint, preferredLine]);

  const visibleMaterials = materials.filter((material) => colorLineForMaterial(material) === activeLine);

  return (
    <>
      <div className="showroom-material-toolbar">
        <label className="showroom-color-line-picker">
          <span>Color line</span>
          <select aria-label="Color line" onChange={(event) => setActiveLine(event.target.value)} value={activeLine}>
  ...
  return (
    <aside aria-label="Design selections and estimate" className="showroom-quote-region workspace-control-surface">
      <section aria-labelledby="showroom-material-heading" className="showroom-materials">
        <ShowroomColorLineShelf materials={materials} />
      </section>
