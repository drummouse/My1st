...
import { projectExtrasOnly, projectServiceLinesOnly } from './lib/trimServiceBoundary.js';
...
export function AppWorkspace({
  ...
  const detailsStateClass = (workspaceState.mode === 'sales' && salesViewModel.detailsOpen === false)
    ...
    ? ' is-details-closed'
    : '';
  const projectSetupClass = workspaceState.mode === 'sales' && salesViewModel.projectSetup === true
    ? ' is-project-setup'
    : '';
  return (
    <div className={`workspace-root app-workspace-layout ${workspaceState.mode}-workspace${safeStateClass}${detailsStateClass}${projectSetupClass}`} data-studio-skin="ironwrap" data-workspace-menu-root="true">
      {shell}
...
export default function App({ currentUser = null }) {
  ...
  const [showroomSelectedServiceId, setShowroomSelectedServiceId] = useState(null);
  ...
  const showroomViewModel = useMemo(() => {
    const canEditCatalog = presentationEditable && !isCustomerView;
    const categories = [
      ...
      { key: 'accents', label: 'Trims & Accents', available: canEditCatalog, unavailableReason: 'Trim geometry is not available in this 3D model.' },
      { key: 'doors', label: 'Garage Doors', available: canEditCatalog, unavailableReason: 'Door geometry is not available in this 3D model.' },
      { key: 'gutters', label: 'Gutters', available: canEditCatalog, unavailableReason: 'Gutter geometry is not available in this 3D model.' },
      { key: 'services', label: 'Additional Services', available: canEditCatalog, unavailableReason: 'Services are available while presenting this design.' },
    ];
    ...
    const rawProductOptions = [
      ...
      ...libraryOptions.services.map((option) => presentationCatalogOption(option, 'services', {
        kind: 'service',
    ...
    const presentationServiceLines = projectServiceLinesOnly(customServiceLines);
    const selectedServiceLine = presentationServiceLines.find((line) => (
      (line.sourceOptionId ?? line.id) === showroomSelectedServiceId
    ))
      ?? presentationServiceLines.find((line) => line.selected !== false)
      ?? presentationServiceLines[0];
    const selectedProductId = showroomSelectedCategory === 'roof'
      ...
      : showroomSelectedCategory === 'siding'
        ...
        : showroomSelectedCategory === 'services'
          ? selectedServiceLine?.selected === false ? undefined : selectedServiceLine?.sourceOptionId ?? selectedServiceLine?.id
    ...
    const selectedProfile = showroomSelectedCategory === 'roof'
      ? roofProfile
      : showroomSelectedCategory === 'siding' ? wallProfile
        : showroomSelectedCategory === 'services' ? undefined : selectedTrimRecord?.profile;
    const selectedColorId = showroomSelectedCategory === 'roof'
      ? roofColorId
      : showroomSelectedCategory === 'siding' ? wallColorId
        : showroomSelectedCategory === 'services' ? undefined : selectedTrimRecord?.colorId;
    ...
    const categoryCanChooseFinish = showroomSelectedCategory !== 'services' && (categoryIsRenderable
    ...
    const addOrSelectProduct = (optionId) => {
      ...
      if (showroomSelectedCategory === 'services') {
        setShowroomSelectedServiceId(option.id);
        setCustomServiceLines((current) => {
          const hasSelection = projectServiceLinesOnly(current).some((line) => (
            (line.sourceOptionId ?? line.id) === option.id
          ));
          return hasSelection
            ? current
            : normalizeCustomServiceLines([...current, libraryOptionToCustomServiceLine(option)]);
        });
        return;
      }
    ...
    const removeProduct = () => {
      ...
      } else if (showroomSelectedCategory === 'services' && selectedServiceLine) {
        const removedId = selectedServiceLine.sourceOptionId ?? selectedServiceLine.id;
        const remaining = presentationServiceLines.filter((line) => (
          (line.sourceOptionId ?? line.id) !== removedId
        ));
        setCustomServiceLines((current) => current.filter((line) => (
          (line.sourceOptionId ?? line.id) !== removedId
        )));
        setShowroomSelectedServiceId(remaining[0]?.sourceOptionId ?? remaining[0]?.id ?? null);
    ...
    const changeProfile = (profile) => {
      ...
      else if (showroomSelectedCategory !== 'services') replaceSelectedTrim({ profile });
    };
    const changeColor = (colorId) => {
      ...
      else if (showroomSelectedCategory !== 'services') replaceSelectedTrim({ colorId });
    };
  ...
  }, [approvedAt, catalogRenderVersion, currentProjectId, customServiceLines, effectiveMaterialsCatalog, estimate.total, hasRoofFaces, hasWallFaces, isCustomerView, libraryOptions.products, libraryOptions.services, measurements.capFlashingLf, measurements.downspoutLf, measurements.fasciaLf, measurements.garageDoorCappingLf, measurements.gutterLf, measurements.soffitSqft, presentationEditable, publicQuoteTotal, roofColorId, roofProductId, roofProfile, services.roof, services.wall, showroomSelectedCategory, showroomSelectedServiceId, showroomSelectedTrimKind, showroomShareTarget.unavailableReason, showroomShareTarget.url, trimAccents, wallColorId, wallProductId, wallProfile]);
  ...
  const salesViewModel = {
    ...
    projectSetup: activeStudioStep === 'project',
    onDetailsOpenChange: setMobileInspectorOpen,
    steps: STUDIO_STEPS,
    activeStep: activeStudioStep,
  ...
  return (
    <div className="app" style={{ '--brand-accent': brand.accent, '--brand-accent-dark': brand.accentDark }}>
      {administrativeWorkspace ? (
        <div className="workspace-root admin-workspace" data-studio-skin="ironwrap" data-workspace-menu-root="true">
          <AdminWorkspaceShell
