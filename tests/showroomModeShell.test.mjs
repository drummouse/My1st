...
test('Showroom constructs strict DTOs before any child receives public props', () => {
  ...
  for (const material of viewModel.materials) {
    assert.deepEqual(Object.keys(material).sort(), [
      'color', 'description', 'id', 'label', 'onSelect', 'selected', 'series', 'thumbnail',
    ]);
  }
...
test('material swatches are selected explicitly and never render internal fields', () => {
  ...
  let renderer;
  act(() => {
    renderer = TestRenderer.create(createElement(ShowroomColorLineShelf, {
      materials: materialViewModel.materials,
    }));
  });
  const swatches = renderer.root.findAll((node) => (
    node.type === 'button' && node.props.className?.includes('showroom-material-swatch')
  ));
...
test('Showroom color-line picker filters the visible swatches by product color line', () => {
  const selected = [];
  const lineMaterials = [
    { id: 'ice', label: 'Ice', color: '#ddd', series: 'Icecrystal Relief', selected: true, onSelect: () => selected.push('ice') },
    { id: 'wood', label: 'Wood', color: '#97674d', series: 'Printech Woodgrain', onSelect: () => selected.push('wood') },
    { id: 'wood-dark', label: 'Wood Dark', color: '#666', series: 'Printech Woodgrain', onSelect: () => selected.push('wood-dark') },
  ];
  let renderer;
  act(() => {
    renderer = TestRenderer.create(createElement(ShowroomColorLineShelf, { materials: lineMaterials }));
  });
  const select = renderer.root.findByProps({ 'aria-label': 'Color line' });
  assert.equal(select.props.value, 'Icecrystal Relief');
  assert.equal(renderer.root.findAll((node) => (
    node.type === 'button' && node.props.className?.includes('showroom-material-swatch')
  )).length, 1);
  act(() => select.props.onChange({ target: { value: 'Printech Woodgrain' } }));
  const swatches = renderer.root.findAll((node) => (
    node.type === 'button' && node.props.className?.includes('showroom-material-swatch')
  ));
  assert.equal(swatches.length, 2);
  swatches[0].props.onClick();
  assert.deepEqual(selected, ['wood']);
  act(() => renderer.unmount());
});
...
test('customer categories cover roof, siding, trims, doors, gutters, and additional services', async () => {
  const app = await readFile(new URL('../src/App.jsx', import.meta.url), 'utf8');
  for (const [key, label] of [
    ['roof', 'Roof'], ['siding', 'Siding'], ['accents', 'Trims & Accents'], ['doors', 'Garage Doors'], ['gutters', 'Gutters'], ['services', 'Additional Services'],
  ]) {
