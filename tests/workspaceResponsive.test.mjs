...
test('mobile positioning leaves a geometric camera-row clearance above its bottom-anchored sheet', async () => {
  ...
  assert.match(sheetRule || '', /bottom:\s*0\.5rem/);
  assert.match(sheetRule || '', /top:\s*auto/);
  assert.match(sheetRule || '', /position:\s*absolute/);
  assert.match(sheetRule || '', /max-height:\s*min\(/);
  assert.match(sheetRule || '', /42dvh/);
  assert.match(sheetRule || '', /17rem/);
  assert.match(sheetRule || '', /width:\s*min\(17rem, calc\(100% - 1rem\)\)/);
  assert.match(sheetRule || '', /overflow-y:\s*auto/);
  assert.match(sheetRule || '', /z-index:\s*40/);
  assert.match(css, /\.workspace-root \.viewer-direction-controls\s*\{[^}]*inset:\s*0[^}]*z-index:\s*4/s);
...
test('closing Sales details restores the flexible viewer row without reserving a blank details row', async () => {
  ...
  assert.match(shell, /onDetailsOpenChange/);
  assert.ok(mobile, 'mobile rules should exist');
  assert.match(mobile, /\.workspace-root\.app-workspace-layout\.sales-workspace\.is-details-closed\s*\{[^}]*"mobile"[^}]*"viewer"[^}]*grid-template-rows:\s*auto minmax\(0, 1fr\)/s);
  assert.match(mobile, /\.sales-workspace\.is-details-closed \.sales-workspace-inspector\s*\{[^}]*display:\s*none/s);
  assert.match(mobile, /\.sales-workspace\.is-project-setup > \.app-workspace-persistent-viewer\s*\{[^}]*display:\s*none/s);
});
