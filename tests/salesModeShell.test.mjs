...
test('Option A composes one compact desktop workspace in the approved region order', async () => {
  ...
  assert.match(source, /className=\{`\$\{embedded \? 'workspace-shell' : 'workspace-root'\} sales-workspace\$\{detailsOpen \? '' : ' is-details-closed'\}\$\{projectSetup \? ' is-project-setup' : ''\}`\}/);
  assert.match(source, /action=\{!detailsOpen && typeof onDetailsOpenChange === 'function'/);
